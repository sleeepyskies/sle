...
public:
     ...
     */
    maybe<TileMap> load(ref<AssetManager> &am, const std::string &mapName);

    /// @brief Saves the given map to disk at the default map location.
    bool save(const std::string &mapName, const hashmap<glm::i8vec2, Chunk> &chunks,
...
private:
    ...
    bool readChunkTiles(std::ifstream &inFile, Chunk &chunk, std::vector<ref<Texture>> &tileTextures,
                        ref<AssetManager> &am);
};
